...
export class MemberService {
  ...
  public async updateMemberByAdmin(
    ...
    const isMainMember = await this.memberModel.findById(memberId).exec();

    if (String(memberId) === String(id)) {
      throw new InternalServerErrorException(Message.BAD_REQUEST);
    } else if (
      member.mainMember ||
      (member.memberType === MemberType.ADMIN && !isMainMember.mainMember)
    ) {
      throw new InternalServerErrorException(Message.BAD_REQUEST);
    }
